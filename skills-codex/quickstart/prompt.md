...
3. Be explicit that Codex has no startup or session-end hook surface under `~/.codex`, so entry skills ensure `ao codex start` once per thread and dedicated closeout skills ensure `ao codex stop` once per thread.
