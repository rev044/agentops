...
if command -v ao >/dev/null 2>&1 && [ -d "$WORKTREE_PATH" ]; then
    (cd "$WORKTREE_PATH" && timeout_run 5 ao know inject --apply-decay --max-tokens 1000 2>/dev/null) || {
        echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) HOOK_WARN: ao know inject failed in worktree $WORKTREE_PATH" \
            >> "$ROOT/.agents/ao/hook-errors.log" 2>/dev/null
    }
fi
