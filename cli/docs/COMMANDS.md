...
> Auto-generated by `scripts/generate-cli-reference.sh` on 2026-02-16.
...
### `ao export-constraints`

Export the project's GOALS.yaml fitness constraints in machine-readable format.

```
ao export-constraints [flags]
```
...
```
      ...
      --archive   Move expired files to archive (requires --expire)
...
Orchestrate the full RPI lifecycle by spawning a fresh Claude session per phase.
...
```
      ...
      --from string       Start from phase (research, plan, pre-mortem, crank, vibe, post-mortem) (default "research")
