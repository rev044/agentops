...
[Any prerequisites: Langfuse enabled, secrets configured, migrations run, etc.]
...
```bash
/crank bd-epic-id              # Full auto until epic closed
```

**Supervised (sequential, same session):**
```bash
/implement-wave bd-epic-id     # One wave at a time
```
