...
Gas Town is a steam engine. You are a piston. The entire system's throughput
...
Every Gas Town agent follows this protocol:
...
- **Pinned**: Permanent reference beads. Does NOT trigger execution.
...
```bash
gt hook attach <mail-id>             # Hook existing mail as assignment
gt handoff -m "..."                  # Create and hook new instructions
```

If you find mail on your hook (not a regular bead), the principle still applies:
read the mail content, interpret the prose instructions, and execute them.
...
demonstrates capability over time.

## Execute With Care

The human slung you work because they trust the engine. Honor that trust.

Quality matters. The Propulsion Principle is about **starting immediately**,
not about rushing through sloppily. Execute with:

- Thoroughness (understand before acting)
- Care (quality over speed)
- Autonomy (don't ask permission for what's clearly assigned)
- Communication (update beads, leave notes for next agent)

## When NOT To Execute

The Propulsion Principle has boundaries:

1. **Ambiguous instructions** - If truly unclear what's being asked, ask
2. **Dangerous operations** - Irreversible actions deserve confirmation
3. **Scope creep** - The bead defines scope; don't expand without asking

But these are exceptions. The default is: **execute**.
