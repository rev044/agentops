...
test_help "ao extract --help" "$AO" extract --help
...
test_help "ao mail --help" "$AO" mail --help
...
test_help "ao context --help" "$AO" context --help
test_help "ao session --help" "$AO" session --help
...
EXPECTED_COMMANDS=(
    version status doctor search inject lookup
    badge forge pool gate
    goals plans ratchet rpi session
    completion config hooks
    inbox mail
    anti-patterns constraint contradict curate dedup
    flywheel maturity memory metrics notebook trace
    extract vibe-check
)
