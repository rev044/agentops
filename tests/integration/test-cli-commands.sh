...
# Test 3: ao search
test_command "$TMPBIN know search test" "ao search \"test\""

# Test 4: ao ratchet status
test_command "$TMPBIN work ratchet status" "ao ratchet status"

# Test 5: ao flywheel status
test_command "$TMPBIN quality flywheel status" "ao flywheel status" "velocity|health|status"

# Test 6: ao pool list
test_command "$TMPBIN quality pool list" "ao pool list"
...
test_command "$TMPBIN know forge --help" "ao forge --help" "forge|transcript"
...
# Test 10: ao inject
test_command "$TMPBIN know inject" "ao inject"

# Test 11: ao ratchet (help — subcommands require args)
test_command "$TMPBIN work ratchet --help" "ao ratchet --help" "ratchet|status|record|Usage"

# Test 12: ao rpi status
test_command "$TMPBIN work rpi status" "ao rpi status"

# Test 13: ao pool promote (help — requires arg)
test_command "$TMPBIN quality pool promote --help" "ao pool promote --help" "promote|usage|Usage"

# Test 14: ao ratchet record (help — requires step name)
test_command "$TMPBIN work ratchet record --help" "ao ratchet record --help" "record|usage|Usage"

# Test 15: ao rpi (help — shows subcommands)
test_command "$TMPBIN work rpi --help" "ao rpi --help" "rpi|status|phased|Usage"
