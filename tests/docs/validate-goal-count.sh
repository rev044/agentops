...
# Extract README claim (line like "44 measurable goals")
readme_claim=$(grep -oE '[0-9]+ measurable goals' "$REPO_ROOT/README.md" | head -1 | grep -oE '^[0-9]+')
...
if [[ -z "$readme_claim" ]]; then
    echo "FAIL: Could not extract goal count from README.md"
    exit 1
fi
