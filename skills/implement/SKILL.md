...
Record implementation progress via ao ratchet:

```bash
# Before starting implementation - claim the issue
ao ratchet claim <issue-id>

# After successful implementation - record completion
ao ratchet record implement \
  --input "issue:<issue-id>" \
  --output "commits:$(git log -1 --format=%H)"

# On failure - record for retry
ao ratchet fail <issue-id> --reason "error description"
```

The ratchet ensures each completed issue is permanently locked as progress.
