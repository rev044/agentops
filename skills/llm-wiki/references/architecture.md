...
- **Runs**: nightly, on a schedule (cron / systemd / launchd / Claude Code hooks)
- **Jobs**:
...
- **The scheduler** — scheduling Tier 1 nightly jobs is host-specific (cron on Linux, launchd on macOS, Task Scheduler on Windows, Claude Code hooks for always-on sessions).
