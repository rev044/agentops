...
In the default `manual` startup mode, MEMORY.md is auto-loaded by Claude Code and no startup injection occurs. Use `/inject` or `ao know inject` for on-demand retrieval when you need deeper context.
...
**Note:** In the default `manual` mode, MEMORY.md is auto-loaded by Claude Code and this hook emits only a pointer to on-demand retrieval commands (`ao know search`, `ao know lookup`).
