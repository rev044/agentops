...
| Deterministic validation authority (`ol validate stage1`) | Olympus | AO | Compile | CLI invocation + JSON output | `STATE_CHECKPOINT_HANDOFF` | `IMPLEMENTED` | `SPEC_ONLY` | Deterministic-first pipeline authority |
| Qualitative council semantics and reporting | AO | Olympus | Compile (consumer) | AO `/council` reports | `INVOCATION_ENVELOPE` | `IMPLEMENTED` | `SPEC_ONLY` | AO owns council prompt/report semantics |
...
| Knowledge compilation to tests/constraints | Olympus | AO | Compile, Chiron | `internal/constraints/*` | `STATE_CHECKPOINT_HANDOFF` | `IMPLEMENTED` | `SPEC_ONLY` | Olympus constraint engine authority |
...
| Constraint export (`ao export-constraints`) | AO | Olympus | Moirai (producer), Compile (consumer) | N/A | `INVOCATION_ENVELOPE` | `FUTURE` | `SPEC_ONLY` | **Phase 2 prerequisite** (bead `ag-q7n` follow-up) |
