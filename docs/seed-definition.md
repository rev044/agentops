...
**Why it exists:** The flywheel needs a place to write. Without `.agents/`, `ao know forge` has nowhere to put learnings, `ao know inject` has nothing to retrieve, and knowledge dies with each session. This is the physical `K` stock from the equation.
...
| SessionEnd | session-end | Extract learnings (`ao know forge`), expire stale artifacts |
| Stop | stop | Close the feedback loop (`ao quality flywheel close-loop`) |

**Why it exists:** Hooks are Meadows #5 -- structural rules. Without hooks, knowledge extraction depends on the agent remembering to run `ao know forge`. Agents forget. Hooks do not. The flywheel only turns automatically if hooks enforce the extract-inject cycle.
...
```markdown
## Knowledge Flywheel
Run `ao know inject` at session start. Run `ao know forge` at session end.
```
...
**Why it exists:** A flywheel with zero learnings is a flywheel that has never turned. The bootstrap learning ensures `ao know inject` has something to retrieve on the very first session. It primes the reinforcing loop (Meadows #7) so the system starts compounding immediately instead of running one empty cycle first.
