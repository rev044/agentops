...
AgentOps is a complete, standalone system — autonomous within a session. You do not need Olympus to use AgentOps. The 36 skills, 12 hooks, knowledge flywheel, and RPI lifecycle work independently — `/rpi` ships features end-to-end, `/evolve` runs goal-driven improvement loops, and the flywheel compounds knowledge across sessions, all without any external daemon.
